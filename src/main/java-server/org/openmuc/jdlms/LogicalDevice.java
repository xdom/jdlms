...
public class LogicalDevice {
    ...
    private void setSystemTitle(String manufacturerId, long deviceId) {
        ...
        for (int i = 0; i < 5; i++) {
            this.systemTitle[i + manIdLength] = (byte) ((byte) 0xff & (deviceId >> ((4 - i) * 8)));
        }
    }
    ...
    public LogicalDevice setConformance(ConformanceSetting... conformanceSetting) {
        if (conformanceSetting.length == 0) {
            conformanceSetting = ConformanceSetting.values();
        }

        this.conformance = new HashSet<>(Arrays.asList(conformanceSetting));
